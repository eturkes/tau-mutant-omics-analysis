...
data_dir <- file.path(
  ...
  "08_Batch_2_StarSolo_mapping", "S3_2_3Solo.out", "Gene", "raw"
)
...
vars_to_regress <- "percent_ribo" # See `cluster_pipeline` in `utils.R`.
...
seurat <- subset(seurat, subset = nFeature_RNA >= 200 & nFeature_RNA < 4000 & percent_mt < 5 & percent_ribo < 10)
VlnPlot(seurat, features = c("nFeature_RNA", "nCount_RNA", "percent_mt", "percent_ribo"), ncol = 4)
seurat
```
...
seurat <- cluster_pipeline(
  ...
  vars_to_regress = vars_to_regress, parallel_override = parallel_override, cc = FALSE, res_divider = 1000000
)
seurat
...
remove <- which(seurat$seurat_clusters == 0)
seurat <- seurat[ , -remove]

sub_name <- "prep_lowqual_removed"
seurat <- cluster_pipeline(
  seurat, cache_dir = cache_dir, sub_name = sub_name, protocol = protocol,
...
seurat[["SCT"]] <- NULL
seurat@meta.data[grep("SCT", colnames(seurat@meta.data))] <- NULL
remove <- which(seurat$seurat_clusters == 8 | seurat$seurat_clusters == 3 | seurat$seurat_clusters == 9)
seurat <- seurat[ , -remove]

sub_name <- "prep_lowqual_removed2"
seurat <- cluster_pipeline(
  seurat, cache_dir = cache_dir, sub_name = sub_name, protocol = protocol,
  vars_to_regress = vars_to_regress, parallel_override = parallel_override, cc = FALSE, res_divider = 10000
)
...
seurat <- RenameIdents(
  object = seurat, "0" = "Oligodendrocyte", "1" = "Microglia", "2" = "Cortical Glutamatergic", "3" = "Hippocampal Glutamatergic",
  "4" = "Astrocyte", "5" = "Dentate Gyrus Glutamatergic", "6" = "GABAergic", "7" = "Endothelial",
  "8" = "Unknown"
)
seurat$annotations <- seurat@active.ident
red_dim_plot(seurat, x = "umap1", y = "umap2", color = "annotations", type = "cat")

rds <- file.path(cache_dir, "seurat_final.rds")
if (!file.exists(rds)) {
