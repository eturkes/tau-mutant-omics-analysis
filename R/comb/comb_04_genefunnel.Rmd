---
title: "Combined Samples - 04 GeneFunnel Analysis"
author:
  - name: "Emir Turkes"
...
knit: (function(inputFile, encoding) {
  rmarkdown::render(
    ...
    output_file = file.path("..", "..", "results", unlist(strsplit(getwd(), "/"))[6], "comb_04_genefunnel.html")
  )})
---
...
# -----------------------------------------------------
library(conflicted)
conflicts_prefer(base::intersect, DT::JS, edgeR::cpm, AnnotationDbi::select, .quiet = TRUE)
packages <- c(
  ...
  "igraph", "qgraph", "circlize", "parallelly"
)
...
if (file.exists(rds)) {
  ...
  keep <- filterGeneSets(gene_sets, min.sz = 5, max.sz = 50)
...
if (file.exists(rds) & file.exists(rds2)) {
  ...
  add <- unique(gene_anno$external_gene_name[which(gene_anno$external_gene_name %notin% rownames(seurat))])
  new_mat <- GetAssayData(seurat, "counts")
  null_mat <- matrix(0, nrow = length(add), ncol = ncol(new_mat))
  for (i in seq(nrow(null_mat))) {
    set.seed(i)
    null_mat[i, sample(ncol(null_mat), size = 1)] <- 1
  }
  ...
  gene_anno <- gene_anno[match(rownames(new_mat), gene_anno$external_gene_name), ]
  rownames(new_mat) <- gene_anno$ensembl_gene_id

  seurat <- CreateSeuratObject(new_mat, meta.data = seurat[[]])
  rm(new_mat)

  seurat$condition <- factor(paste("Microglia", seurat$sample, sep = "_"))
...
sce_filt <- as.SingleCellExperiment(seurat[rowSums(as.matrix(GetAssayData(seurat, slot = "counts")) > 0) > 0, ])
...
if (file.exists(rds)) {
  ...
  gsva <- gsva(
    logcounts(sce), gene_sets, method = "ssgsea", parallel.sz = availableCores(), ssgsea.norm = FALSE, verbose = FALSE
  )
  colnames(gsva) <- sce$condition
  saveRDS(gsva, rds)
}
# -------------------------------
```
...
colnames(design) <- levels(sce$condition)
...
# ----------------------------------------------------
tests <- decideTests(cont_fit, p.value = 5e-25)
tests_up <- tests[tests[ , 1] == 1, ]
tests_down <- tests[tests[ , 1] == -1, ]
results <- topTable(cont_fit, number = Inf)
...
colnames(design) <- levels(sce_filt$condition)
...
for (j in seq(length(gene_sets_sub_up@.Data))) {
  if (
    length(which(gene_sets_sub_up[[j]]@geneIds %in% markers_list_pseudobulk_all[[2]])) >=
      round(length(gene_sets_sub_up[[j]]@geneIds) / 6)
    ) {
...
for (j in seq(length(gene_sets_sub_down@.Data))) {
  if (
    length(which(gene_sets_sub_down[[j]]@geneIds %in% markers_list_pseudobulk_all[[3]])) >=
      round(length(gene_sets_sub_down[[j]]@geneIds) / 6)
  ) {
...
draw(
  Heatmap(
    ...
    row_names_gp = gpar(fontsize = 7, col = unlist(row_colors)),
...
png(file.path(results_dir, "comb_04_genefunnel_network.png"), 150, 100, "in", res = 72)
...
plot(
    ...
    edge.width = (E(graph)$weight * 1.25) ^ 2.25,
    vertex.shape = "pie",
    ...
    vertex.label.cex = ifelse(DE == "yes", ((all_DE_match$Freq) * 3), 3),
