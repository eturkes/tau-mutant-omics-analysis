...
for (j in seq(length(gene_sets_sub_up@.Data))) {
  if (
    length(which(gene_sets_sub_up[[j]]@geneIds %in% markers_list_pseudobulk_all[[2]])) >=
      ceiling(length(gene_sets_sub_up[[j]]@geneIds) / 9)
    ) {
...
for (j in seq(length(gene_sets_sub_down@.Data))) {
  if (
    length(which(gene_sets_sub_down[[j]]@geneIds %in% markers_list_pseudobulk_all[[3]])) >=
      ceiling(length(gene_sets_sub_down[[j]]@geneIds) / 9)
  ) {
...
# )
module_membership_thresh <- median(abs(rowSums(module_membership_norm)))
...
membership <- abs(rowSums(module_membership_norm))
...
plot(
    ...
    vertex.size = (membership + 1) ^ 1.5,
    vertex.label.color = vertex_label_color,
    edge.width = (E(graph)$weight * 0.7) ^ 3,
    vertex.shape = "pie",
    ...
    vertex.label.cex = ifelse(DE == "yes", ((all_DE_match$Freq) * 0.75), 0.75),
