...
# -------------------------------
rds <- file.path(cache_dir, "gsva_orig.rds")
rds2 <- file.path(cache_dir, "gsva_filt.rds")
rds3 <- file.path(cache_dir, "gsva.rds")
if (file.exists(rds)) {
  gsva_orig <- readRDS(rds)
  gsva_filt <- readRDS(rds2)
  gsva <- readRDS(rds3)
} else {
  gsva_orig <- gsva(
  ...
  saveRDS(gsva_orig, rds)
  saveRDS(gsva_filt, rds2)
  saveRDS(gsva, rds3)
}
# -------------------------------

hist(gsva_orig, main = "gsva_orig distribution")
hist(gsva_filt, main = "gsva_filt distribution")
...
for (j in seq(length(gene_sets_sub_up@.Data))) {
  if (
    length(which(gene_sets_sub_up[[j]]@geneIds %in% markers_list_pseudobulk_all[[2]])) >=
      ceiling(length(gene_sets_sub_up[[j]]@geneIds) / 10)
    ) {
...
for (j in seq(length(gene_sets_sub_down@.Data))) {
  if (
    length(which(gene_sets_sub_down[[j]]@geneIds %in% markers_list_pseudobulk_all[[3]])) >=
      ceiling(length(gene_sets_sub_down[[j]]@geneIds) / 10)
  ) {
...
membership <- (abs(rowSums(module_membership_norm)) / hub_gene_all_GO) * hub_gene_all_GO
...
png(file.path(results_dir, paste0("comb_04_genefunnel_network_", analysis_name, ".png")), width = 5280, height = 4320, res = 300)
...
plot(
    ...
    vertex.size = (membership + 0.75) ^ 2,
    vertex.label.color = vertex_label_color,
    edge.width = (E(graph)$weight * 0.75) ^ 3,
    vertex.shape = "pie",
    ...
    vertex.label.cex = ifelse(DE == "yes", ((all_DE_match$Freq) * 1), 1),
