---
title: "MAPT KI vs. P301S+3 - Batch 02 - 01 GeneFunnel"
author:
  - name: "Emir Turkes"
...
if (file.exists(rds)) {
  ...
  file_list <- c(file_list[1], file_list[5])
...
cont_mat <- makeContrasts(MAPTKI-P301S, levels = design)
...
for (i in seq_along(unique(groups))) {
  if (i == 1) {
    gene <- rownames(results)[results$Coef > 0.1]
    avg_log2FC <- results$Coef[results$Coef > 0.1]
  } else {
    gene <- rownames(results)[results$Coef < -0.1]
    avg_log2FC <- abs(results$Coef)[results$Coef < -0.1]
  }
  add <- data.frame(
...
rownames(results) <- results$X
results <- results[ , -1]
results_up <- results[results$Coef > 0.1, ]
results_down <- results[results$Coef < -0.1, ]
...
for (i in seq_along(top_genes_left)) {
  ...
  #   avg_log2FC <- avg_log2FC[avg_log2FC > 0.1]
  # } else {
