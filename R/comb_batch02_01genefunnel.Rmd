...
rds <- file.path(cache_dir, "markers_gsva_sample.rds")
if (file.exists(rds)) {
  markers <- readRDS(rds)
} else {
...
seurat_rm_unknown <- seurat[ , -remove]
seurat_rm_unknown$sample_ctype <- factor(
  paste(seurat_rm_unknown$sample, seurat_rm_unknown$annotations_short, sep = " ")
)
seurat_rm_unknown@active.ident <- seurat_rm_unknown$sample_ctype

sce <- as.SingleCellExperiment(seurat_rm_unknown, assay = "GSVA")
sce <- suppressWarnings(
  aggregateAcrossCells(sce, colData(sce)[ , "sample_ctype"], use_exprs_values = "counts", statistics = "sum")
)
...
rds <- file.path(cache_dir, "markers_gsva_sample_ctype.rds")
if (file.exists(rds)) {
  markers <- readRDS(rds)
} else {
  markers <- FindAllMarkers(seurat_rm_unknown, only.pos = TRUE)
  saveRDS(markers, rds)
}
