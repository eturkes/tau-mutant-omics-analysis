...
for (i in seq_along(unique(groups))) {
  gene <- rownames(gse_zscore)[
    rowMeans(gse_zscore[ , group_idx[[i]]]) > 0
  ]
...
for (i in seq_along(top_words_list)) {
  ...
  if (length(tmp) >= 3) {
    ...
    tmp <- suppressWarnings(word_cloud(top_words_tmp_sub$gene_set, 3, dict = dict))
  }
...
for (i in seq_along(top_genes)) {
  ...
  # counts_zscore_scaled <- t(
  #   apply(counts_zscore, MARGIN = 1, FUN = function (x) ((2 * (x - min(x)) / (max(x) - min(x))) - 1))
  # )
  counts_zscore_scaled <- as.matrix((counts_zscore - min(counts_zscore)) / (max(counts_zscore) - min(counts_zscore)))
  # counts_zscore_scaled <- (2 * (counts_zscore - min(counts_zscore)) / (max(counts_zscore) - min(counts_zscore))) - 1
  counts_zscore_scaled <- counts_zscore_scaled[counts_zscore[ , i] > 0, ]
  counts_zscore_sub <- counts_zscore_scaled[rownames(counts_zscore_scaled) %in% names(genes), i]
  # counts_zscore_sub <- counts_zscore_sub[counts_zscore_sub > 0.5]
  ...
  matches <- table(
    factor(names(genes), levels = names(genes))[match(unlist(geneIds(gene_sets)), names(genes), nomatch = 0)]
  )
  matches <- (matches - max(matches)) / (min(matches) - max(matches))

  comb <- genes * counts_zscore_sub * matches
...
gene_ids <- gene_ids[names(gene_ids) %in% top_gse[[1]]]
gene_ids <- gene_ids[match(top_gse[[1]], names(gene_ids))]
...
gene_ids <- geneIds(gene_sets)
gene_ids <- gene_ids[names(gene_ids) %in% top_gse[[2]]]
gene_ids <- gene_ids[match(top_gse[[2]], names(gene_ids))]
links_orig <- data.frame(
  source = rep(names(gene_ids), times = lengths(gene_ids)), target = gene_ids %>% map_dfr(as_tibble)
)
map <- setNames(gene_anno$external_gene_name, gene_anno$ensembl_gene_id)
...
gene_ids <- geneIds(gene_sets)
gene_ids <- gene_ids[names(gene_ids) %in% top_gse[[3]]]
gene_ids <- gene_ids[match(top_gse[[3]], names(gene_ids))]
links_orig <- data.frame(
  source = rep(names(gene_ids), times = lengths(gene_ids)), target = gene_ids %>% map_dfr(as_tibble)
)
map <- setNames(gene_anno$external_gene_name, gene_anno$ensembl_gene_id)
...
links$value <- links_orig$value
colnames(links) <- c("source", "target")
links <- links[links$target %in% unique(names(top_genes[[3]])), ]
links$value <- rep(1, times = nrow(links))
dup <- which(duplicated(links[ , 1:2]))
if (length(dup) > 0) {
  links <- links[-dup, ]
...
gene_ids <- geneIds(gene_sets)
gene_ids <- gene_ids[names(gene_ids) %in% top_gse[[4]]]
gene_ids <- gene_ids[match(top_gse[[4]], names(gene_ids))]
links_orig <- data.frame(
  source = rep(names(gene_ids), times = lengths(gene_ids)), target = gene_ids %>% map_dfr(as_tibble)
)
map <- setNames(gene_anno$external_gene_name, gene_anno$ensembl_gene_id)
...
links$value <- links_orig$value
colnames(links) <- c("source", "target")
links <- links[links$target %in% unique(names(top_genes[[4]])), ]
links$value <- rep(1, times = nrow(links))
dup <- which(duplicated(links[ , 1:2]))
if (length(dup) > 0) {
  links <- links[-dup, ]
