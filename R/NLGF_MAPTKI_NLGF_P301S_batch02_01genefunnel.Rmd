---
title: "NLGF MAPT KI vs. NLGF P301S+3 - Batch 02 - 01 GeneFunnel"
author:
  - name: "Emir Turkes"
...
# Useful variables.
# -----------------
genotype <- "NLGF_MAPTKI_NLGF_P301S"
batch <- 2
step <- 1
...
cont_mat <- makeContrasts(NLGF_MAPTKI-NLGF_P301S, levels = design)
  ...
  for (l in 2:26) {
    ...
    for (m in seq_along(modules)) {
      ...
      if (length(modules_up) == 0) {
        ...
        module_membership_norm_tmp <- module_membership_norm_calc[
    ...
    modules_with_hubs[[l]] <- length(which(lengths(hub_gene_list) >= 1)) / l
  }
...
for (m in seq_along(modules)) {
  ...
  if (length(modules_up) == 0) {
    ...
    # module_membership_norm_tmp <- module_membership_norm_tmp[module_membership_norm_tmp >= module_membership_thresh]
    module_hub_genes <- names(module_membership_norm_tmp)
    module_hub_genes <- module_hub_genes[module_hub_genes %in% rownames(results_list[[1]])]
  }

  hub_gene_list_left[[m]] <- module_hub_genes
}
...
for (m in seq_along(modules)) {
  ...
  if (length(modules_up) == 0) {
    ...
    # module_membership_norm_tmp <- module_membership_norm_tmp[module_membership_norm_tmp >= module_membership_thresh]
    module_hub_genes <- names(module_membership_norm_tmp)
    module_hub_genes <- module_hub_genes[module_hub_genes %in% rownames(results_list[[2]])]
  }

  hub_gene_list_right[[m]] <- module_hub_genes
}
...
for (i in seq_along(top_genes_left)) {
  ...
  genes <- genes[names(genes) %in% unlist(hub_gene_list_left)]
...
for (i in seq_along(top_genes_right)) {
  ...
  genes <- genes[names(genes) %in% unlist(hub_gene_list_right)]
...
unique_direction$down <- unique_direction$down * -1
module_membership_norm <- unique_direction
module_membership_norm <- module_membership_norm[abs(rowSums(module_membership_norm)) >= 3, ]
