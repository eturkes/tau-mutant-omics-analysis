...
for (i in seq_along(unique(groups))) {
  if (i == 1) {
    gene <- rownames(results)[results$Coef > 0.2]
    avg_log2FC <- results$Coef[results$Coef > 0.2]
  } else {
    gene <- rownames(results)[results$Coef < -0.2]
    avg_log2FC <- abs(results$Coef)[results$Coef < -0.2]
  }
  add <- data.frame(
...
rownames(results) <- results$X
results <- results[ , -1]
results_up <- results[results$Coef > 0.2, ]
results_down <- results[results$Coef < -0.2, ]
...
module_membership_norm <- module_membership_norm[abs(rowSums(module_membership_norm)) >= 2, ]
    ...
    for (m in seq_along(modules)) {
      ...
      if (length(modules_up) == 0) {
        ...
        module_hub_genes_tmp <- module_hub_genes_tmp[
          module_hub_genes_tmp$ensembl_gene_id %in% rownames(results_up),
        ]
        # module_hub_genes_tmp <- gene_anno[
        #   gene_anno$ensembl_gene_id %in% rownames(results_up),
        # ]
...
row_anno_left <-  HeatmapAnnotation(
  top_words_left = anno_block(
    gp = gpar(col = module_colours), labels = seq(length(unique(optimal_k_left))),
...
for (i in seq_along(top_genes_left)) {
  ...
  #   avg_log2FC <- avg_log2FC[avg_log2FC > 0.2]
  # } else {
  ...
  counts_cpm_sub <- counts_cpm[rownames(counts_cpm) %in% names(avg_log2FC_scaled), , drop = FALSE]
...
row_anno_right <-  HeatmapAnnotation(
  top_words_right = anno_block(
    gp = gpar(col = module_colours), labels = seq(length(unique(optimal_k_right))),
    labels_rot = 0, labels_gp = gpar(fontsize = 9)
  ),
  which = "row"
)
...
panel_fun <- function(index, nm) {
  ...
  grid.rect(
    ...
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][2]))
  )
  grid.rect(
    x = 0, y = 0.875, width = top_genes_left[[nm]][3] / 10,
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][3]))
  )
  grid.rect(
    x = 0, y = 0.625, width = top_genes_left[[nm]][5] / 10,
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][5]))
  )
  grid.rect(
    x = 0, y = 0.625, width = top_genes_left[[nm]][6] / 10,
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][6]))
  )
  grid.rect(
    x = 0, y = 0.375, width = top_genes_left[[nm]][8] / 10,
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][8]))
  )
  grid.rect(
    x = 0, y = 0.375, width = top_genes_left[[nm]][9] / 10,
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][9]))
  )
  grid.rect(
    x = 0, y = 0.125, width = top_genes_left[[nm]][11] / 10,
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][11]))
  )
  grid.rect(
    x = 0, y = 0.125, width = top_genes_left[[nm]][12] / 10,
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][12]))
  )

  grid.text(y = 0.875, names(top_genes_left[[nm]])[1], gp = gpar(fontsize = 7), x = 0.02, just = "left")
  grid.text(y = 0.625, names(top_genes_left[[nm]])[4], gp = gpar(fontsize = 7), x = 0.02, just = "left")
  grid.text(y = 0.375, names(top_genes_left[[nm]])[7], gp = gpar(fontsize = 7), x = 0.02, just = "left")
  grid.text(y = 0.125, names(top_genes_left[[nm]])[10], gp = gpar(fontsize = 7), x = 0.02, just = "left")
  grid.xaxis(gp = gpar(fontsize = 4), main = FALSE)
  popViewport()
}
right_anno <- rowAnnotation(
  foo = anno_link(
    align_to = optimal_k_left, which = "row", panel_fun = panel_fun, link_width = unit(0, "mm"), gap = unit(3, "mm")
  )
)
...
panel_fun <- function(index, nm) {
  ...
  grid.rect(
    ...
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][2]))
  )
  grid.rect(
    x = 0, y = 0.875, width = top_genes_right[[nm]][3] / 10,
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][3]))
  )
  grid.rect(
    x = 0, y = 0.625, width = top_genes_right[[nm]][5] / 10,
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][5]))
  )
  grid.rect(
    x = 0, y = 0.625, width = top_genes_right[[nm]][6] / 10,
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][6]))
  )
  grid.rect(
    x = 0, y = 0.375, width = top_genes_right[[nm]][8] / 10,
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][8]))
  )
  grid.rect(
    x = 0, y = 0.375, width = top_genes_right[[nm]][9] / 10,
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][9]))
  )
  grid.rect(
    x = 0, y = 0.125, width = top_genes_right[[nm]][11] / 10,
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][11]))
  )
  grid.rect(
    x = 0, y = 0.125, width = top_genes_right[[nm]][12] / 10,
    height = 0.25, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][12]))
  )

  grid.text(y = 0.875, names(top_genes_right[[nm]])[1], gp = gpar(fontsize = 7), x = 0.02, just = "left")
  grid.text(y = 0.625, names(top_genes_right[[nm]])[4], gp = gpar(fontsize = 7), x = 0.02, just = "left")
  grid.text(y = 0.375, names(top_genes_right[[nm]])[7], gp = gpar(fontsize = 7), x = 0.02, just = "left")
  grid.text(y = 0.125, names(top_genes_right[[nm]])[10], gp = gpar(fontsize = 7), x = 0.02, just = "left")
  grid.xaxis(gp = gpar(fontsize = 4), main = FALSE)
  popViewport()
}
right_anno <- rowAnnotation(
  foo = anno_link(
    align_to = optimal_k_right, which = "row", panel_fun = panel_fun, link_width = unit(0, "mm"), gap = unit(3, "mm")
  )
)
...
```{r}
ctype <- "Dentate Gyrus Glutamatergic"
...
panel_fun <- function(index, nm) {
  ...
  grid.rect(
    ...
    height = 1, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][2]))
  )
  grid.rect(
    x = 0, y = 0.5, width = top_genes_left[[nm]][3] / 10,
    height = 1, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][3]))
  )

  grid.text(y = 0.5, names(top_genes_left[[nm]])[1], gp = gpar(fontsize = 7), x = 0.02, just = "left")
  grid.xaxis(gp = gpar(fontsize = 4), main = FALSE)
  popViewport()
}
right_anno <- rowAnnotation(
...
panel_fun <- function(index, nm) {
  ...
  grid.rect(
    ...
    height = 1, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][2]))
  )
  grid.rect(
    x = 0, y = 0.5, width = top_genes_right[[nm]][3] / 10,
    height = 1, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][3]))
  )

  grid.text(y = 0.5, names(top_genes_right[[nm]])[1], gp = gpar(fontsize = 7), x = 0.02, just = "left")
  grid.xaxis(gp = gpar(fontsize = 4), main = FALSE)
  popViewport()
}
right_anno <- rowAnnotation(
...
links$value <- links_orig$value
colnames(links) <- c("source", "target")
links <- links[links$target %in% unique(sapply(top_genes_left, function(x) names(x)[1])), ]
dup <- which(duplicated(links[ , 1:2]))
if (length(dup) > 0) {
  links <- links[-dup, ]
}
...
links$value <- links_orig$value
colnames(links) <- c("source", "target")
links <- links[links$target %in% unique(sapply(top_genes_right, function(x) names(x)[1])), ]
dup <- which(duplicated(links[ , 1:2]))
if (length(dup) > 0) {
  links <- links[-dup, ]
}
