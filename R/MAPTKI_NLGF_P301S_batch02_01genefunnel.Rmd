  ...
  for (l in 4:26) {
    ...
    for (m in seq_along(modules)) {
      ...
      if (length(modules_up) == 0) {
        ...
        frequent_genes$Freq <- frequent_genes$Freq * tmp * (frequent_genes$Freq / nrow(mat)) * (frequent_genes$Freq / length(modules_up))
...
for (m in seq_along(modules)) {
  ...
  if (length(modules_up) == 0) {
    ...
    frequent_genes$Freq <- abs(frequent_genes$Freq)
    frequent_genes <- frequent_genes[order(frequent_genes$Freq, decreasing = TRUE), ]
    frequent_genes <- frequent_genes[frequent_genes$Var1 %in% rownames(results_list[[1]]), ]
  }
...
for (m in seq_along(modules)) {
  ...
  if (length(modules_up) == 0) {
    ...
    frequent_genes$Freq <- abs(frequent_genes$Freq)
    frequent_genes <- frequent_genes[order(frequent_genes$Freq, decreasing = TRUE), ]
    frequent_genes <- frequent_genes[frequent_genes$Var1 %in% rownames(results_list[[2]]), ]
  }
