...
optimal_k <- vector("numeric", length = length(levels(groups)))
...
for (i in seq_along(levels(groups))) {
  ...
  for (l in 2:13) {
    ...
    dict_list <- vector("list", length = length(modules))
    for (j in seq_along(dict_list)) {
      dict_list[[j]] <- suppressWarnings(word_cloud(rownames(mat)[modules[[j]]]))
    ...
    word_list <- vector("list", length = length(modules))
    for (j in seq_along(word_list)) {

      for (k in seq_along(modules[[j]])) {
        word_list[[j]][[k]] <- suppressWarnings(word_cloud(rownames(mat)[modules[[j]][k]], dict = dict))
        word_list[[j]][[k]] <- strsplit(paste(unlist(word_list[[j]][[k]]), collapse = " "), "\\s+")[[1]]
      }

      modules_with_hubs[[l]][[j]] <- table(unlist(word_list[[j]]))
      modules_with_hubs[[l]][[j]] <- modules_with_hubs[[l]][[j]][order(modules_with_hubs[[l]][[j]], decreasing = TRUE)]
    }
  }
  stats <- vector("numeric", length = length(modules_with_hubs))
  for (j in seq_along(stats)) {
    if (j != 1) {
      scores <- vector("numeric", length = length(modules_with_hubs[[j]]))
      for (k in seq_along(scores)) {
        scores[k] <- max(modules_with_hubs[[j]][[k]])
      }
      stats[j] <- mean(scores)
    }
  }
  optimal_k[i] <- 14 - which.max(rev(stats))
}
