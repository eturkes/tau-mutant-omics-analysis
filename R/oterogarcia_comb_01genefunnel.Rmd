...
for (i in seq_along(levels(groups))) {
  ...
  for (l in 2:26) {
    ...
    for (m in seq_along(modules)) {
      ...
      cutoff <- quantile(frequent_genes$Freq, probs = 0.75)
      hub_gene_list[[m]] <- frequent_genes$Freq[frequent_genes$Freq >= cutoff]
    ...
    if (any(lengths(hub_gene_list) == 0)) {
      ...
      modules_with_hubs[[l]] <- median(unlist(hub_gene_list))
    }
  }
...
for (m in seq_along(modules)) {
  ...
  frequent_genes <- frequent_genes[frequent_genes$Var1 %in% rownames(results_sig), ]
  cutoff <- quantile(frequent_genes$Freq, probs = 0.75)
  hub_gene_list_left[[m]] <- as.vector(frequent_genes$Var1)[frequent_genes$Freq >= cutoff]
...
for (m in seq_along(modules)) {
  ...
  frequent_genes <- frequent_genes[frequent_genes$Var1 %in% rownames(results_sig), ]
  cutoff <- quantile(frequent_genes$Freq, probs = 0.75)
  hub_gene_list_right[[m]] <- as.vector(frequent_genes$Var1)[frequent_genes$Freq >= cutoff]
...
for (i in seq_along(top_words_list)) {
  ...
  ) >= ceiling(length(hub_gene_list_left[[i]]) / 8)
...
for (i in seq_along(top_words_list)) {
  ...
  )  >= ceiling(length(hub_gene_list_right[[i]]) / 8)
  top_words_tmp_sub <- top_words_tmp_sub[idx, ]
...
#   hub_gene_data_list[[i]] <- quantile(hub_gene_data_sub$hub_score, probs = 0.9)
# }
# 
# keep <- c()
# for (i in seq(nrow(hub_gene_data))) {
#   gene_set_name <- hub_gene_data$module[i]
#   if (hub_gene_data$hub_score[i] >= hub_gene_data_list[[gene_set_name]]) {
#     keep <- c(keep, i)
#   }
# }
...
layout <- qgraph.layout.fruchtermanreingold(
  ...
  area = vcount(graph) / 1, repulse.rad = vcount(graph) / 1
)
...
png(file.path(
  results_dir, paste0("oterogarcia_comb_01genefunnel_network_left.png")), width = 4680, height = 4500, res = 300
)
...
jam_igraph(
    ...
    vertex.size = sqrt(hub_score * 1e5) * 1.5,
    ...
    vertex.label.cex = ifelse(DE == "yes", 2, 1),
...
for (l in seq_along(order_right)) {
  ...
  node_data$module <- l
  node_data$hub <- ifelse(
    node_data$ensembl_gene_id %in% hub_gene_right$x, "yes", "no"
  )
  node_data$top_gse <- ifelse(
...
png(file.path(
  results_dir, paste0("oterogarcia_comb_01genefunnel_network_right.png")), width = 4680, height = 4500, res = 300
)
par(bg = "black", mar = c(0, 0, 0, 8), xpd = TRUE)
jam_igraph(
    graph,
