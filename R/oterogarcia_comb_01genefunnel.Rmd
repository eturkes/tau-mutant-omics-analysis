...
## BA9 Layer 2/3 EX Neurons

```{r, fig.width = 8, fig.height = 6, dpi = 72}
ctype <- "BA9 Layer 2/3 EX Neurons"
...
for (i in seq_along(top_words_list)) {
  ...
  idx <- colSums(
    sapply(genes, "%in%", x = c(hub_gene_list_left[[i]][1], NA, NA))
  ) > 0
  ...
  tmp <- tmp[order(tmp, decreasing = TRUE)]
  if (length(tmp) > round(nrow(top_words_tmp_sub) / 5)) {
    tmp <- tmp[seq(round(nrow(top_words_tmp_sub) / 5))]
  }
  if (length(tmp) == 1) {
    tmp[1] <- 4
  } else {
...
for (i in seq_along(top_genes_left)) {
  ...
  comb <- comb[names(comb) %in% hub_gene_list_left[[i]][1]]
  hub_gene_list_sub <- hub_gene_list_left[[i]][1][
    hub_gene_list_left[[i]][1] %in% names(comb)
  ]
...
for (i in seq_along(top_words_list)) {
  ...
  genes <- geneIds(genes[match(modules[[i]], names(genes))])
  idx <- colSums(
    sapply(genes, "%in%", x = c(hub_gene_list_right[[i]][1], NA, NA))
  ) > 0
  top_words_tmp_sub <- top_words_tmp_sub[idx, ]
...
for (i in seq_along(top_genes_right)) {
  ...
  comb <- genes_int * avg_log2FC_scaled_int * matches_sub_int
  comb <- comb[names(comb) %in% hub_gene_list_right[[i]][1]]
  hub_gene_list_sub <- hub_gene_list_right[[i]][1][
    hub_gene_list_right[[i]][1] %in% names(comb)
  ]
  comb <- comb[match(hub_gene_list_sub, names(comb))]
  # comb <- c(comb, avg_log2FC_scaled_int[names(avg_log2FC_scaled_int) %notin% names(comb)][1])
  # comb <- c(comb, genes_int[names(genes_int) %notin% names(comb)][1])
