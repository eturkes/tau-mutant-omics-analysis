...
for (i in seq(nrow(hub_gene_data))) {
  ...
  if (hub_gene_data$hub_score[i] >= quantile(hub_gene_data$hub_score, probs = 0.75) | hub_gene_data$hub[i] == "yes") {
    keep <- c(keep, i)
  }
}
...
unique_markers <- gene_anno[gene_anno$ensembl_gene_id %in% hub_gene_left$x, ]
DE <- vector()
for (l in seq(length(V(graph)))) {
...
jam_igraph(
    ...
    vertex.size = sqrt(sqrt(hub_score * 1e7)),
...
unique_markers <- gene_anno[gene_anno$ensembl_gene_id %in% hub_gene_right$x, ]
DE <- vector()
for (l in seq(length(V(graph)))) {
  if (V(graph)[l]$name %in% unique_markers$external_gene_name) {
    DE <- append(DE, "yes")
