...
for (i in seq_along(hub_gene_data_list)) {
  ...
  hub_gene_data_list[[i]] <- median(hub_gene_data_sub$hub_score)
}

keep <- c()
for (i in seq(nrow(hub_gene_data))) {
  gene_set_name <- hub_gene_data$gene_set[i]
  if (hub_gene_data$hub_score[i] >= hub_gene_data_list[[gene_set_name]] | hub_gene_data$DE[i] == "yes") {
    keep <- c(keep, i)
  }
}
