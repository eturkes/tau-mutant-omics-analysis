...
keep <- filterGeneSets(gene_sets, min.sz = 2, max.sz = 25)
...
# hub_gene_data_sub <- hub_gene_data[hub_gene_data$DE == "yes", ]
# hub_gene_data <- hub_gene_data[hub_gene_data$hub_score >= min(hub_gene_data_sub$hub_score), ]
# hub_gene_data <- hub_gene_data[hub_gene_data$hub_score >= 0.0001, ]
...
layout <- qgraph.layout.fruchtermanreingold(
  ...
  area = vcount(graph), repulse.rad = vcount(graph) / 1
)
...
png(file.path(
  results_dir, paste0("oterogarcia_comb_01genefunnel_network_left.png")), width = 5280, height = 4320, res = 300
)
...
jam_igraph(
    ...
    vertex.size = sqrt(sqrt(hub_score * 1e5)),
    vertex.label.color = vertex_label_color,
    edge.width = (E(graph)$weight * 0.5) ^ 2,
    vertex.shape = "pie",
    ...
    vertex.label.cex = ifelse(DE == "yes", 1, 0.5),
...
png(file.path(
  results_dir, paste0("oterogarcia_comb_01genefunnel_network_right.png")), width = 5280, height = 4320, res = 300
)
par(bg = "black", mar = c(0, 0, 0, 18), xpd = TRUE)
jam_igraph(
    graph,
