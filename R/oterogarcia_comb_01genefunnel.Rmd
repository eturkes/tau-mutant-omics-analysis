...
packages <- c(
  ...
  "DT", "circlize", "purrr", "networkD3", "factoextra", "reshape2", "dendextend", "data.table", "scales"
)
...
groups <- factor(
  sub(".*?(\\w+)-\\w+$", "\\1", colnames(gse_cpm)),
  levels = unique(sub(".*?(\\w+)-\\w+$", "\\1", colnames(gse_cpm)))
)
...
for (i in seq_along(levels(groups))) {
  ...
  for (l in 2:26) {
    ...
    for (m in seq_along(modules)) {
      ...
      hub_gene_list[[m]] <- frequent_genes$Freq[1:ceiling(length(modules_up) / 15)]
...
for (m in seq_along(modules)) {
  ...
  hub_gene_list_left[[m]] <- as.vector(frequent_genes$Var1)[1:ceiling(length(modules_up) / 15)]
...
for (m in seq_along(modules)) {
  ...
  hub_gene_list_right[[m]] <- as.vector(frequent_genes$Var1)[1:ceiling(length(modules_up) / 15)]
...
for (i in seq_along(top_words_list)) {
  ...
  ) >= ceiling(length(hub_gene_list_left[[i]]) / 5)
  ...
  tmp <- tmp[order(tmp, decreasing = TRUE)]
  if (length(tmp) > ceiling(nrow(top_words_tmp_sub) / 5)) {
    tmp <- tmp[seq(ceiling(nrow(top_words_tmp_sub) / 5))]
  }
  if (length(tmp) == 1) {
    tmp[1] <- 4
  } else {
...
for (j in seq_along(top_gse_left)) {
  ...
  for (i in seq_along(top_gse_left[[j]])) {
    ...
    ) >= ceiling(length(hub_gene_list_left[[j]]) / 5)
...
for (i in seq_along(top_genes_left)) {
  ...
  names(top_genes_left[[i]]) <- c(add, rep("darkgray", times = length(add)), rep("lightgray", times = length(add)))
...
for (i in seq_along(top_words_list)) {
  ...
  )  >= ceiling(length(hub_gene_list_right[[i]]) / 5)
  top_words_tmp_sub <- top_words_tmp_sub[idx, ]
...
for (j in seq_along(top_gse_right)) {
  ...
  for (i in seq_along(top_gse_right[[j]])) {
    ...
    ) >= ceiling(length(hub_gene_list_right[[j]]) / 5)
...
for (i in seq_along(top_genes_right)) {
  ...
  names(top_genes_right[[i]]) <- c(add, rep("darkgray", times = length(add)), rep("lightgray", times = length(add)))
...
panel_fun <- function(index, nm) {
  ...
  if (length(top_genes_left[[nm]]) == 3) {
    ...
    grid.rect(
      ...
      height = 0.5, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][5]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.75, width = top_genes_left[[nm]][3] / 10,
      height = 0.5, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][3]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.25, width = top_genes_left[[nm]][6] / 10,
      height = 0.5, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][6]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.25, width = top_genes_left[[nm]][4] / 10,
      height = 0.5, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][4]), lwd = 0.5)
    )
    ...
    grid.rect(
      ...
      height = 1 / 3, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][7]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.5 + 1 / 3, width = top_genes_left[[nm]][4] / 10,
      height = 1 / 3, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][4]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.5, width = top_genes_left[[nm]][8] / 10,
      height = 1 / 3, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][8]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.5, width = top_genes_left[[nm]][5] / 10,
      height = 1 / 3, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][5]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.5 - 1 / 3, width = top_genes_left[[nm]][9] / 10,
      height = 1 / 3, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][9]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.5 - 1 / 3, width = top_genes_left[[nm]][6] / 10,
      height = 1 / 3, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][6]), lwd = 0.5)
    )
    grid.text(
    ...
    grid.rect(
      ...
      height = 0.25, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][10]), lwd = 0.5)
    )
    grid.rect(
    ...
    grid.rect(
      ...
      height = 0.25, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][7]), lwd = 0.5)
    )
    grid.rect(
...
right_anno <-  HeatmapAnnotation(
  top_genes = anno_link(
    ...
    link_width = unit(0, "mm"), gap = unit(0.75, "mm"), width = unit(6, "mm")
  ),
  which = "row",
  clusters = anno_block(
...
draw(
  Heatmap(
    ...
    heatmap_legend_param = list(
      ...
      legend_width = unit(1.25, "cm"), title_gp = gpar(fontsize = 3.5), labels_gp = gpar(fontsize = 3),
      grid_height = unit(0.075, "cm"), at = c(0, 2, 4, 6, 8, 10)
    )
  ),
...
panel_fun <- function(index, nm) {
  ...
  if (length(top_genes_right[[nm]]) == 3) {
    ...
    grid.rect(
      ...
      height = 0.5, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][3]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.75, width = top_genes_right[[nm]][5] / 10,
      height = 0.5, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][5]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.25, width = top_genes_right[[nm]][4] / 10,
      height = 0.5, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][4]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.25, width = top_genes_right[[nm]][6] / 10,
      height = 0.5, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][6]), lwd = 0.5)
    )
    ...
    grid.rect(
      ...
      height = 1 / 3, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][4]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.5 + 1 / 3, width = top_genes_right[[nm]][7] / 10,
      height = 1 / 3, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][7]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.5, width = top_genes_right[[nm]][5] / 10,
      height = 1 / 3, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][5]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.5, width = top_genes_right[[nm]][8] / 10,
      height = 1 / 3, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][8]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.5 - 1 / 3, width = top_genes_right[[nm]][6] / 10,
      height = 1 / 3, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][6]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.5 - 1 / 3, width = top_genes_right[[nm]][9] / 10,
      height = 1 / 3, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][9]), lwd = 0.5)
    )
    grid.text(
    ...
    grid.rect(
      ...
      height = 0.25, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][10]), lwd = 0.5)
    )
    grid.rect(
      x = 0, y = 0.375, width = top_genes_right[[nm]][7] / 10,
      height = 0.25, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][7]), lwd = 0.5)
    )
    grid.rect(
