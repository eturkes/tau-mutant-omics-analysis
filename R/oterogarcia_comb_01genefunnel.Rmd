...
#   hub_gene_data_list[[i]] <- hub_gene_data_sub$hub
# }
#
...
# hub_gene_data <- hub_gene_data[hub_gene_data$hub_score >= 0.00001, ]
...
layout <- qgraph.layout.fruchtermanreingold(
  ...
  area = vcount(graph) / 1, repulse.rad = vcount(graph) / 2
)
...
png(file.path(
  results_dir, paste0("oterogarcia_comb_01genefunnel_network_left.png")), width = 900, height = 720, res = 150
)
par(bg = "black", mar = c(0, 0, 0, 4), xpd = TRUE)
jam_igraph(
    graph,
    rescale = FALSE,
    xlim = c(-1, 1),
    ...
    vertex.size = sqrt(sqrt(hub_score * 1e8)),
    ...
    vertex.label.cex = ifelse(DE == "yes", 0.5, 0.5),
...
png(file.path(
  results_dir, paste0("oterogarcia_comb_01genefunnel_network_right.png")), width = 900, height = 720, res = 150
)
par(bg = "black", mar = c(0, 0, 0, 4), xpd = TRUE)
jam_igraph(
    graph,
