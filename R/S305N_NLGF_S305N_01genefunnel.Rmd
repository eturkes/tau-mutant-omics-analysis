...
for (i in seq_along(top_genes_left)) {
  ...
  comb <- comb[match(hub_gene_list_left[[i]][1:round(length(which(optimal_k_left == i)) / 10)], names(comb))]
...
for (i in seq_along(top_genes_right)) {
  ...
  comb <- comb[match(hub_gene_list_right[[i]][1:round(length(which(optimal_k_right == i)) / 10)], names(comb))]
  # comb <- c(comb, avg_log2FC_scaled_int[names(avg_log2FC_scaled_int) %notin% names(comb)][1])
  # comb <- c(comb, genes_int[names(genes_int) %notin% names(comb)][1])
  # comb <- c(comb, matches_sub_int[names(matches_sub_int) %notin% names(comb)][1])
...
top_anno <- HeatmapAnnotation(
  groups = anno_block(
    gp = gpar(fill = c("lightgray", "lightgray"), lwd = 0.5),
...
right_anno <- rowAnnotation(
  word_cloud = anno_textbox(
    ...
    padding = unit(c(0.5, 0.1, 0.25, 1), "mm"),
...
draw(
  Heatmap(
    ...
    rect_gp = gpar(col = "darkgray", lwd = 0.01),
...
mat_left_sub <- mat_left[rownames(mat_left) %in% unlist(top_gse_left), ]
mat_left_sub <- mat_left_sub[match(unique(unlist(top_gse_left)), rownames(mat_left_sub)), ]

module_idx <- sapply(rownames(mat_left_sub), FUN = find_index, list_of_lists = top_gse_left)
...
draw(
  Heatmap(
    mat_left_sub,
    col = colour,
    column_labels = sub("\\s+[^ ]+$", "", colnames(mat_left_sub)),
    cluster_columns = FALSE,
    cluster_rows = FALSE,
    column_split = factor(groups, levels = unique(groups)),
    row_split = module_idx,
    show_column_names = FALSE,
    left_annotation = left_anno,
    column_title = ctype,
    column_title_gp = gpar(fontsize = 5),
    ...
    row_names_max_width = max_text_width(rownames(mat_left_sub)),
    row_names_side = "left",
    row_gap = unit(0.5, "mm"),
    column_gap = unit(0.5, "mm"),
    column_names_side = "top",
...
mat_right_sub <- mat_right[rownames(mat_right) %in% unlist(top_gse_right), ]
mat_right_sub <- mat_right_sub[match(unique(unlist(top_gse_right)), rownames(mat_right_sub)), ]

module_idx <- sapply(rownames(mat_right_sub), FUN = find_index, list_of_lists = top_gse_right)
...
draw(
  Heatmap(
    mat_right_sub,
    col = colour,
    column_labels = sub("\\s+[^ ]+$", "", colnames(mat_right_sub)),
    cluster_columns = FALSE,
    cluster_rows = FALSE,
    column_split = factor(groups, levels = unique(groups)),
    row_split = module_idx,
    ...
    row_title = NULL,
    row_names_gp = gpar(fontsize = 3),
    row_names_max_width = max_text_width(rownames(mat_right_sub)),
    row_names_side = "left",
    row_gap = unit(0.5, "mm"),
    column_gap = unit(0.5, "mm"),
    column_names_side = "top",
