---
title: "S305N+3 vs. NLGF S305N+3 - Batch 02 - 01 GeneFunnel"
author:
  - name: "Emir Turkes"
...
knit: (function(inputFile, encoding) {
  rmarkdown::render(
    ...
    output_file = file.path("..", "results", "S305N_NLGF_S305N_batch02_01genefunnel.html")
  )})
---
...
# Useful variables.
# -----------------
genotype <- "S305N_NLGF_S305N"
batch <- "comb"
step <- 1

cache_dir <- file.path("..", "cache", genotype, batch, paste0("0", step))
...
if (file.exists(rds)) {
  ...
  file_list <- c(file_list[7], file_list[8], file_list[4], file_list[5])
  files <- vector("list", length = length(file_list))
  for (i in seq_along(files)) {
    files[[i]] <- counts(readRDS(file.path(cache_dir, "..", "..", "..", file_list[i])))
    colnames(files[[i]]) <- paste(
      colnames(files[[i]]), sub("\\/.*", "", file_list[i]),
      sapply(strsplit(file_list[i], "/"), function(x) x[2]), sep = " "
    )
  }
...
keep <- grep(ctype, x = colnames(gse))
gse_cpm <- log1p(cpm(gse[ , keep]))
counts_cpm <- log1p(cpm(counts[ , keep]))

groups <- factor(
  sapply(strsplit(colnames(gse_cpm), "\\s+"), `[`, 2),
  levels = unique(sapply(strsplit(colnames(gse_cpm), "\\s+"), `[`, 2))
)
design <- model.matrix(~ 0 + groups)
colnames(design) <- unique(groups)

fit <- lmFit(gse_cpm, design)
cont_mat <- makeContrasts(S305N-NLGF_S305N, levels = design)
  ...
  for (l in 6:26) {
    ...
    for (m in seq_along(modules)) {
      ...
      if (length(modules_up) == 0) {
        ...
        tmp <- (abs(rowSums(module_membership_norm_sub)) / hub_gene_all_GO_sub) * rowSums(module_membership_norm_sub)
        ...
        if (i == 1) {
          remove <- which(frequent_genes$Freq <= 0)
          if (length(remove) > 0) {
      ...
      hub_gene_list[[m]] <- frequent_genes$Freq[1]
    }
    modules_with_hubs[[l]] <- min(unlist(hub_gene_list))
  }
...
for (m in seq_along(modules)) {
  ...
  if (length(modules_up) == 0) {
    ...
    module_gene_sets_sub <- gene_sets[names(gene_sets) %in% modules_up]
    frequent_genes <- as.data.frame(table(unlist(geneIds(module_gene_sets_sub))))
    frequent_genes <- frequent_genes[frequent_genes$Var1 %in% rownames(results_up), ]

    results_sub <- results_up[rownames(results_up) %in% frequent_genes$Var1, ]
    results_sub <- results_sub[match(frequent_genes$Var1, rownames(results_sub)), ]
    results_sub <- abs(results_sub$Coef)
    results_sub <- (results_sub - min(results_sub)) / (max(results_sub) - min(results_sub))
  ...
  hub_gene_list_left[[m]] <- as.vector(frequent_genes$Var1)[1]
}
...
for (m in seq_along(modules)) {
  ...
  if (length(modules_up) == 0) {
    ...
    module_gene_sets_sub <- gene_sets[names(gene_sets) %in% modules_up]
    frequent_genes <- as.data.frame(table(unlist(geneIds(module_gene_sets_sub))))
    frequent_genes <- frequent_genes[frequent_genes$Var1 %in% rownames(results_down), ]

    results_sub <- results_down[rownames(results_down) %in% frequent_genes$Var1, ]
    results_sub <- results_sub[match(frequent_genes$Var1, rownames(results_sub)), ]
    results_sub <- abs(results_sub$Coef)
    results_sub <- (results_sub - min(results_sub)) / (max(results_sub) - min(results_sub))
  ...
  hub_gene_list_right[[m]] <- as.vector(frequent_genes$Var1)[1]
}
...
markers_tmp$cluster <- unlist(lapply(seq_along(order_left), function(i) {rep(names(order_left)[i], length(order_left[[i]]))}))
...
for (i in seq_along(top_words_list)) {
  ...
  idx <- colSums(sapply(genes, "%in%", x = c(hub_gene_list_left[[i]], NA, NA))) > 0
  ...
  top_words_list[[i]] <- vector("list", nrow(top_words_tmp_sub))
  for (j in seq_along(top_words_list[[i]])) {
    top_words_list[[i]][[j]] <- suppressWarnings(word_cloud(top_words_tmp_sub$gene_sets[j], dict = c()))
  }
  tmp <- strsplit(paste(unlist(top_words_list[[i]]), collapse = " "), "\\s+")[[1]]
  remove <- which(tmp == "")
  if (length(remove) > 0) {
  ...
  remove <- c()
  for (j in seq_along(tmp)) {
    dup <- grep(names(tmp)[j], x = names(tmp))
    if (length(dup) > 1) {
  ...
  if (length(tmp) >= 1) {
    ...
    add <- 1 - length(tmp)
    tmp2 <- unlist(strsplit(suppressWarnings(word_cloud(top_words_tmp_sub$gene_set, dict = c())), "\\s+"))
    ...
    tmp <- suppressWarnings(word_cloud(top_words_tmp_sub$gene_set, 1, dict = c()))
  }
...
for (j in seq_along(top_gse_left)) {
  ...
  for (i in seq_along(top_gse_left[[j]])) {
    idx <- c(
      grep(top_words_left[j], x = unlist(top_words_list[[j]]))
    )
    top_gse_left[[j]][[i]] <- modules[[j]][idx]
    top_gse_left_all[[j]][[i]] <- modules[[j]]
    idx <- c(
      grep(top_words_left[j], x = unlist(top_words_list_all[[j]]))
    )
...
for (i in seq_along(top_genes_left)) {
  ...
  top_genes_left[[i]] <- c(
    comb[1], counts_cpm_sub[1, 1], counts_cpm_sub[1, 1 + 1]
  )
...
markers_tmp$cluster <- unlist(lapply(seq_along(order_right), function(i) {rep(names(order_right)[i], length(order_right[[i]]))}))
...
for (i in seq_along(top_words_list)) {
  ...
  genes <- gene_sets[names(gene_sets) %in% modules[[i]]]
  genes <- geneIds(genes[match(modules[[i]], names(genes))])
  idx <- colSums(sapply(genes, "%in%", x = c(hub_gene_list_right[[i]], NA, NA))) > 0
  top_words_tmp_sub <- top_words_tmp_sub[idx, ]
...
for (j in seq_along(top_gse_right)) {
  ...
  for (i in seq_along(top_gse_right[[j]])) {
    idx <- c(
      grep(top_words_right[j], x = unlist(top_words_list[[j]]))
    )
    top_gse_right[[j]][[i]] <- modules[[j]][idx]
    top_gse_right_all[[j]][[i]] <- modules[[j]]
    idx <- c(
      grep(top_words_right[j], x = unlist(top_words_list_all[[j]]))
    )
...
for (i in seq_along(top_genes_right)) {
  ...
  top_genes_right[[i]] <- c(
    comb[1], counts_cpm_sub[1, 2], counts_cpm_sub[1, 2 - 1]
  )
...
top_genes_left_words <- top_genes_left
names(top_genes_left_words) <- top_words_left
...
links <- links[links$target %in% unique(sapply(top_genes_left_words, function(x) names(x)[1])), ]
...
top_genes_right_words <- top_genes_right
names(top_genes_right_words) <- top_words_right
gene_ids <- geneIds(gene_sets)
...
links$value <- links_orig$value
colnames(links) <- c("source", "target")
links <- links[links$target %in% unique(sapply(top_genes_right_words, function(x) names(x)[1])), ]
dup <- which(duplicated(links[ , 1:2]))
if (length(dup) > 0) {
  links <- links[-dup, ]
}
...
gene_ids <- geneIds(gene_sets)
gene_ids <- gene_ids[names(gene_ids) %in% unlist(top_gse_left_words)]
gene_ids <- gene_ids[match(unlist(top_gse_left_words), names(gene_ids))]
links_orig <- data.frame(
  source = rep(names(gene_ids), times = lengths(gene_ids)), target = gene_ids %>% map_dfr(as_tibble)
)
map <- setNames(gene_anno$external_gene_name, gene_anno$ensembl_gene_id)
...
stack <- rbindlist(lapply(top_gse_left_words, FUN = stack))
# stack <- stack[stack$values %in% unique(links$source), ]
stack <- stack[ , c(2, 1)]
colnames(stack) <- colnames(links)
links_orig <- links
...
value <- c()
for (target in stack$target) {
  value <- c(value, length(which(links_orig$source %in% target)) + 1)
}
links$value <- c(rep(2, times = nrow(links_orig)), value)
nodes <- data.frame(name = c(as.character(links$source), as.character(links$target)) %>% unique())
links$IDsource <- match(links$source, nodes$name) - 1
links$IDtarget <- match(links$target, nodes$name) - 1
sankeyNetwork(
  Links = links, Nodes = nodes, Source = "IDsource", Target = "IDtarget",
  Value = "value", NodeID = "name", fontSize = 20, sinksRight = FALSE
)

gene_ids <- geneIds(gene_sets)
gene_ids <- gene_ids[names(gene_ids) %in% unlist(top_gse_right_words)]
gene_ids <- gene_ids[match(unlist(top_gse_right_words), names(gene_ids))]
links_orig <- data.frame(
  source = rep(names(gene_ids), times = lengths(gene_ids)), target = gene_ids %>% map_dfr(as_tibble)
)
map <- setNames(gene_anno$external_gene_name, gene_anno$ensembl_gene_id)
...
stack <- rbindlist(lapply(top_gse_right_words, FUN = stack))
# stack <- stack[stack$values %in% unique(links$source), ]
stack <- stack[ , c(2, 1)]
colnames(stack) <- colnames(links)
links_orig <- links
...
for (i in seq_along(unique(links$target))) {
  links_sub <- links[links$target %in% names(top_genes_left[[i]][1]), ]
  links_sub <- links_sub[links_sub$source %in% top_gse_left_all[[i]][[1]], ]
  if (i == 1) {
    links_pruned <- links_sub
  } else {
    links_pruned <- rbind(links_pruned, links_sub)
  }
}

freq <- table(links_pruned$target)
freq <- freq[match(unique(links_pruned$target), names(freq))]
freq <- freq / lengths(order_left) * 100
for (i in seq_along(top_genes_left)) {
  gene <- names(top_genes_left[[i]][1])
  idx <- which(names(freq) == gene)
  names(top_genes_left[[i]]) <- c(
    paste0(gene, " (", round(freq[[idx]], digits = 2), "%)"),
    names(top_genes_left[[i]][2]), names(top_genes_left[[i]][3])
  )
}
...
for (i in seq_along(unique(links$target))) {
  links_sub <- links[links$target %in% names(top_genes_right[[i]][1]), ]
  links_sub <- links_sub[links_sub$source %in% top_gse_right_all[[i]][[1]], ]
  if (i == 1) {
    links_pruned <- links_sub
  } else {
    links_pruned <- rbind(links_pruned, links_sub)
...
freq <- table(links_pruned$target)
freq <- freq[match(unique(links_pruned$target), names(freq))]
freq <- freq / lengths(order_right) * 100
for (i in seq_along(top_genes_right)) {
  gene <- names(top_genes_right[[i]][1])
  idx <- which(names(freq) == gene)
  names(top_genes_right[[i]]) <- c(
    paste0(gene, " (", round(freq[[idx]], digits = 2), "%)"),
    names(top_genes_right[[i]][2]), names(top_genes_right[[i]][3])
  )
}
```
...
panel_fun <- function(index, nm) {
  ...
  grid.rect(
    ...
    height = 1, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][2]), lwd = 0.5)
  )
  grid.rect(
    x = 0, y = 0.5, width = top_genes_left[[nm]][3] / 10,
    height = 1, just = "left", gp = gpar(fill = names(top_genes_left[[nm]][3]), lwd = 0.5)
  )

  grid.text(y = 0.5, names(top_genes_left[[nm]])[1], gp = gpar(fontsize = 4, font = 3), x = 0.03, just = "left")
...
right_anno <- rowAnnotation(
  foo = anno_link(
    ...
    link_width = unit(0, "mm"), gap = unit(1.75, "mm"), width = unit(12, "mm")
  )
)
...
draw(
  Heatmap(
    ...
    rect_gp = gpar(col = "darkgray", lwd = 0.05),
...
panel_fun <- function(index, nm) {
  ...
  grid.rect(
    ...
    height = 1, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][2]), lwd = 0.5)
  )
  grid.rect(
    x = 0, y = 0.5, width = top_genes_right[[nm]][3] / 10,
    height = 1, just = "left", gp = gpar(fill = names(top_genes_right[[nm]][3]), lwd = 0.5)
  )

  grid.text(y = 0.5, names(top_genes_right[[nm]][1]), gp = gpar(fontsize = 4, font = 3), x = 0.03, just = "left")
  grid.xaxis(gp = gpar(fontsize = 2, lwd = 0.5), main = FALSE)
  popViewport()
}
right_anno <- rowAnnotation(
...
row_anno_right <-  HeatmapAnnotation(
  top_words_right = anno_block(
    gp = gpar(col = module_colors[seq(optimal_k[2])]),
