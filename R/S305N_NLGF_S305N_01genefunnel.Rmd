...
links <- links[links$target %in% unique(sapply(top_genes_left_words, function(x) names(x)[c(1, 4, 7, 10)])), ]
...
links$value <- links_orig$value
colnames(links) <- c("source", "target")
links <- links[links$target %in% unique(sapply(top_genes_right_words, function(x) names(x)[c(1, 4, 7, 10)])), ]
dup <- which(duplicated(links[ , 1:2]))
if (length(dup) > 0) {
  links <- links[-dup, ]
}
...
for (i in seq_along(unique(links_orig$target))) {
  links_sub <- links_orig[links_orig$target %in% names(top_genes_left[[i]][1]), ]
...
for (i in seq_along(unique(links_orig$target))) {
  links_sub <- links_orig[links_orig$target %in% names(top_genes_right[[i]][1]), ]
