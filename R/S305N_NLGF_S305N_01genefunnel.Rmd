...
for (i in seq_along(top_words_list)) {
  ...
  idx <- colSums(
    sapply(genes, "%in%", x = c(hub_gene_list_left[[i]][1:round(length(which(optimal_k_left == i)) / 10)], NA, NA))
  ) > 0
  ...
  tmp <- tmp[order(tmp, decreasing = TRUE)]
  if (length(tmp) > round(nrow(top_words_tmp_sub) / 10)) {
    tmp <- tmp[seq(round(nrow(top_words_tmp_sub) / 10))]
  }
  if (length(tmp) == 1) {
    tmp[1] <- 5
  } else {
    tmp <- (tmp - min(tmp)) / (max(tmp) - min(tmp)) + 4
  }
  # if (length(tmp) >= 1) {
    # tmp <- tmp[1:1]
...
for (i in seq_along(top_genes_left)) {
  ...
  comb <- comb[names(comb) %in% hub_gene_list_left[[i]][1:round(length(which(optimal_k_left == i)) / 10)]]
  hub_gene_list_sub <- hub_gene_list_left[[i]][1:round(length(which(optimal_k_left == i)) / 10)][
    hub_gene_list_left[[i]][1:round(length(which(optimal_k_left == i)) / 10)] %in% names(comb)
  ]
...
for (i in seq_along(top_words_list)) {
  ...
  genes <- geneIds(genes[match(modules[[i]], names(genes))])
  idx <- colSums(
    sapply(genes, "%in%", x = c(hub_gene_list_right[[i]][1:round(length(which(optimal_k_right == i)) / 10)], NA, NA))
  ) > 0
  top_words_tmp_sub <- top_words_tmp_sub[idx, ]
...
for (i in seq_along(top_genes_right)) {
  ...
  comb <- genes_int * avg_log2FC_scaled_int * matches_sub_int
  comb <- comb[names(comb) %in% hub_gene_list_right[[i]][1:round(length(which(optimal_k_right == i)) / 10)]]
  hub_gene_list_sub <- hub_gene_list_right[[i]][1:round(length(which(optimal_k_right == i)) / 10)][
    hub_gene_list_right[[i]][1:round(length(which(optimal_k_right == i)) / 10)] %in% names(comb)
  ]
  comb <- comb[match(hub_gene_list_sub, names(comb))]
  # comb <- c(comb, avg_log2FC_scaled_int[names(avg_log2FC_scaled_int) %notin% names(comb)][1])
  # comb <- c(comb, genes_int[names(genes_int) %notin% names(comb)][1])
...
anno_colour <- hue_pal()(2)
top_anno <- HeatmapAnnotation(
  Replicate = paste0("Replicate ", rep(1:2, times = 2)),
  groups = anno_block(
  ...
  col = list(Replicate = c("Replicate 1" = anno_colour[1], "Replicate 2" = anno_colour[2])),
...
left_anno <-  HeatmapAnnotation(
  ...
  top_genes = anno_link(
    ...
    link_width = unit(0, "mm"), gap = unit(0.75, "mm"), width = unit(5, "mm")
  ),
  which = "row"
)
...
draw(
  Heatmap(
    ...
    border = TRUE,
    row_title = top_words_left_title,
    ...
    heatmap_legend_param = list(
      ...
      legend_width = unit(1.5, "cm"), title_gp = gpar(fontsize = 4), labels_gp = gpar(fontsize = 3),
...
draw(
  Heatmap(
    ...
    border_gp = gpar(lwd = 0.5),
    border = TRUE,
    row_title = top_words_right_title,
    row_title_gp = gpar(fontsize = 4),
    row_title_rot = 0,
    row_gap = unit(0.75, "mm"),
    column_gap = unit(0.5, "mm"),
