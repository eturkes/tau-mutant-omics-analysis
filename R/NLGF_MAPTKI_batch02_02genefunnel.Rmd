---
title: "NLGF MAPT KI - Batch 02 - 02 GeneFunnel"
author:
  - name: "Emir Turkes"
...
for (j in seq_along(top_gse)) {
  ...
  top_gse[[j]] <- rownames(markers)[idx]
}

min <- min(counts(sce))
keep <- rownames(counts(sce)[rowSums(counts(sce) > min) >= 1, ])
sce_sub <- sce[keep, ]
d <- density(counts(sce_sub))
plot(d)

sct_zscores <- t(scale(t(counts(sce_sub))))
d <- density(sct_zscores)
plot(d)
...
for (i in seq_along(top_genes)) {
  ...
  sct_zscores_sub <- sct_zscores[rownames(sct_zscores) %in% names(genes), i]
  genes <- genes[names(genes) %in% names(sct_zscores_sub)]
  sct_zscores_sub <- sct_zscores_sub[match(names(genes), names(sct_zscores_sub))]

  comb <- genes * (sct_zscores_sub + 1)
  # comb <- sct_zscores_sub
  comb <- comb[order(comb, decreasing = TRUE)]

  sct_zscores_sub <- sct_zscores_sub[match(names(comb), names(sct_zscores_sub))]
  genes <- genes[match(names(comb), names(genes))]

  top_genes[[i]] <- c(comb[1], sct_zscores_sub[1], genes[1], comb[2], sct_zscores_sub[2], genes[2])
  names(top_genes[[i]]) <- c(
    rep(
