...
if (file.exists(rds)) {
  ...
  keep <- filterGeneSets(gene_sets, min.sz = 4, max.sz = Inf)
...
rds <- file.path(cache_dir, "sce.rds")
if (file.exists(rds)) {
  sce <- readRDS(rds)
} else {
...
if (file.exists(rds)) {
  ...
  for (j in seq(ncol(sce))) {
    ...
    for (i in seq_along(gene_sets)) {
      ...
      var <- exp(-var(aggr) / 100000)
      if (is.na(var)) {
        var <- 1
      }
      # aggr <- sum(aggr) / total[j]
      aggr <- sum(aggr) / length(geneIds(gene_sets[[i]]))
      score <- aggr * var
      print(c(j, i, aggr[[1]], var, score[[1]]))
      if (var == 0) {
        stop()
      }
      tmp <- rbind(tmp, score)
...
keep <- rownames(gse[rowSums(gse > min(gse)) >= 1, ])
gse_sub <- gse[keep, ]
plot(density(gse_sub))

gse_zscore <- t(scale(t(gse_sub)))
plot(density(gse_zscore))

rds <- file.path(cache_dir, "gse_zscore.rds")
if (!file.exists(rds)) {
  saveRDS(gse_zscore, file = rds)
}
```
...
for (i in seq(ncol(gse_zscore))) {
  gene <- rownames(gse_zscore)[gse_zscore[ , i] > 1.96]
...
min <- min(counts(sce))
keep <- rownames(counts(sce)[rowSums(counts(sce) > min) >= 1, ])
sce_sub <- sce[keep, ]
plot(density(counts(sce_sub)))

sce_zscore <- t(scale(t(counts(sce_sub))))
plot(density(sce_zscore))

rds <- file.path(cache_dir, "sce_zscore.rds")
if (!file.exists(rds)) {
  saveRDS(sce_zscore, file = rds)
}
...
for (i in seq_along(top_genes)) {
  ...
  sce_zscore_sub <- sce_zscore[rownames(sce_zscore) %in% names(genes), i]
  sce_zscore_sub <- sce_zscore_sub[sce_zscore_sub > 0]
  genes <- genes[names(genes) %in% names(sce_zscore_sub)]
  sce_zscore_sub <- sce_zscore_sub[match(names(genes), names(sce_zscore_sub))]

  comb <- genes * sce_zscore_sub
  # comb <- sce_zscore_sub
  # comb <- genes
  comb <- comb[order(comb, decreasing = TRUE)]

  sce_zscore_sub <- sce_zscore_sub[match(names(comb), names(sce_zscore_sub))]
  genes <- genes[match(names(comb), names(genes))]

  top_genes[[i]] <- c(comb[1], sce_zscore_sub[1], genes[1], comb[2], sce_zscore_sub[2], genes[2])
  names(top_genes[[i]]) <- c(
    rep(
...
panel_fun <- function(index, nm) {
  ...
  # grid.rect(
  #   x = 0, y = 0.25, width = top_genes[[nm]][6],
  #   height = 0.5, just = "left", gp = gpar(fill = colour(top_genes[[nm]][5]))
  # )
  grid.text(y = 0.65, names(top_genes[[nm]])[1], gp = gpar(fontsize = 7), x = 1, just = "right")
  # grid.text(y = 0.25, names(top_genes[[nm]])[4], gp = gpar(fontsize = 7), x = 1, just = "right")
