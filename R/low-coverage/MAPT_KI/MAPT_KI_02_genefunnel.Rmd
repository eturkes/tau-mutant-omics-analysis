...
library(conflicted)
conflicts_prefer(
  base::intersect, DT::JS, edgeR::cpm, AnnotationDbi::select, matrixStats::rowMedians, dplyr::filter, .quiet = TRUE
)
packages <- c(
...
#     sce, colData(sce)[ , "seurat_clusters"], use_exprs_values = "counts", statistics = "sum"
#   )
# )
...
```{r}
# pseudobulk_seurat <- CreateSeuratObject(gsva)
# pseudobulk_seurat@active.ident <- factor(colnames(pseudobulk_seurat))
# 
# top_genes <- c()
# for (i in seq(ncol(gsva))) {
#   top_genes <- c(top_genes, rownames(gsva)[order(gsva[ , i], decreasing = TRUE)[1]])
# }
# 
# markers <- FindAllMarkers(pseudobulk_seurat, slot = "counts", only.pos = TRUE)
# top_markers <- markers %>% group_by(cluster) %>% top_n(1, avg_log2FC)
```
...
mat <- gsva[rownames(gsva) %in% rownames(markers), ]
...
draw(
  Heatmap(
    ...
    column_split = seurat$seurat_clusters
  )
)
```
...
if (file.exists(rds)) {
  ...
  seurat_orig <- readRDS(file.path(cache_dir, "..", "01", "all_seurat.rds"))

  anchors <- FindTransferAnchors(
    ref, seurat_orig, normalization.method = "SCT", recompute.residuals = FALSE,
    reference.reduction = "pca", dims = 1:30, verbose = FALSE
  )
  seurat_orig <- MapQuery(anchors, seurat_orig, ref, refdata = "subclass_label", reference.reduction = "umap", verbose = FALSE)
...
seurat$predicted.id <- seurat_orig$predicted.id

table <- table(seurat$seurat_clusters, seurat$predicted.id)
max <- colnames(table)[apply(table,1,which.max)]
seurat$seurat_clusters_predicted <- paste("Cluster", seurat$seurat_clusters, seurat$predicted.id, sep = " ")
```
...
```{r}
marker_genes <- FindAllMarkers(seurat_orig, only.pos = TRUE)
```

```{r, fig.height=10}
c13 <- markers[markers$cluster == "13", ]

c13_mat <- mat[rownames(mat) %in% rownames(c13), ]
...
Heatmap(    t(
    ...
    color,cluster_columns = FALSE,
row_names_max_width = max_text_width(rownames(c13_mat)))
)
