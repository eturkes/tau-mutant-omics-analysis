---
title: "NLGF S305N - Batch 02 - 01 Prep"
author:
  - name: "Emir Turkes"
...
data_dir <- file.path(
  ...
  "08_Batch_2_StarSolo_mapping", "S6_2_6Solo.out", "Gene", "filtered"
)
...
We first read in the data and form SingleCellExperiment and Seurat objects.

```{r}
sce <- SingleCellExperiment(list(counts = Read10X(data_dir)))
...
We use adaptive thresholds to view but not remove cells that are outliers by more than 3 MADs.
StarSolo should have taken care of most filtering already, but we make sure to remove cells with more than 10% mitochondrial reads.

```{r}
sce$discard <- remove$discard
datatable_download(t(colSums(as.matrix(remove))))

if (any(sce$subsets_mito_percent >= 10)) {
  remove <- which(sce$subsets_mito_percent >= 10)
  sce <- sce[ , -remove]
}
...
```{r}
sub_name <- "prep"
seurat <- seurat[rownames(seurat) %in% rownames(sce), colnames(seurat) %in% colnames(sce)]
...
seurat <- cluster_pipeline(
  ...
  vars_to_regress = vars_to_regress, parallel_override = parallel_override, cc = FALSE, res_divider = 75000
)
seurat
